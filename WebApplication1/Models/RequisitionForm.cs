    ...
    public class RequisitionForm
    {
        ...
        public RequisitionForm Create()
        {
            string columns = "INSERT INTO RequisitionForm(Date, Department, Position, "
                             ...
                             + "UnderSupervision, BriefDescriptionofWorks) OUPUT INSERTED.RequisitionID ";
            string values = " Values(@Date, @Department, @Position, @RequestedBy, @ReasonforVacancy, "
                            ...
                            + "@ExpectedJoiningDate, @UnderSupervision, @BriefDescriptionofWorks)";
            ...
            param.Add("@ExpectedJoiningDate", this.ExpectedJoiningDate.ToShortDateString());
        ...
        public RequisitionForm Update(int RequisitionID, bool recursive = true)
        {
            ...
            string set = "UPDATE RequisitionForm SET Date = @Date AND Position = @Position AND "
                         + "RequestedBy = @RequestedBy AND ReasonforVacancy = @ReasonforVacancy"
                         + " AND Type = @Type AND Qualification = @Qualification AND "
                         + "ExperienceRequired = @ExperienceRequired AND "
                         + "SkillsRequired = @SkillsRequired AND Department = @Department AND"
                         + "ExpectedJoiningDate = @ExpectedJoiningDate AND "
                         + "UnderSupervision = @UnderSupervision AND "
                         + "BriefDescriptionofWorks = @BriefDescriptionofWorks";
            Dictionary<string, dynamic> param = new Dictionary<string, dynamic>();
